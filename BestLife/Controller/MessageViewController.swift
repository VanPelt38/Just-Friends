...
class MessageViewController: MessagesViewController {
    ...
    override func viewDidLoad() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(addListener), name: UIApplication.didBecomeActiveNotification, object: nil)
        NotificationCenter.default.addObserver(self, selector: #selector(removeListener), name: UIApplication.willResignActiveNotification, object: nil)
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        UserDefaults.standard.set(true, forKey: "onChatView")
    ...
    @objc func addListener() {
        ...
        if let safeDetails = matchDetails {
            listener = db.collection("chats").document(safeDetails[0].chatID).collection("messages").addSnapshotListener { [weak self] snapshot, error in
                ...
                for change in snapshot.documentChanges {
                    ...
                    if change.type == .added {
                        ...
                        if let time = documentData["timeStamp"] as? Timestamp, let userID = documentData["ID"] as? String, let message = documentData["message"] as? String {
                            ...
                            if messageTime > currentTime && userID != self.firebaseID {
                                
                                guard let realm = RealmManager.getRealm() else {return}
                                
                                if let messageExists = realm.object(ofType: RChatDoc.self, forPrimaryKey: change.document.documentID) {
                                    return
                                } else {
                                    ...
                                    try! realm.write {
                                        ...
                                        newMessage.chatID = safeDetails[0].chatID
                                        realm.add(newMessage)
                                        
                                        let newMess = Message(id: "1", date: messageTime, message: message, sender: Sender(name: safeDetails[0].name, id: userID))
                                        self.finalMessages.append(newMess)
                                    }
    ...
    func getNewChats() async {
        
        if let safeMatchID = matchDetails?[0].chatID {
            
            guard let realm = RealmManager.getRealm() else {return}
            
            let pathway = db.collection("chats").document(safeMatchID).collection("messages")
                ...
                for chat in newChats.documents {
                    ...
                    if let timeStamp = data["timeStamp"] as? Timestamp, let userID = data["ID"] as? String, let message = data["message"] as? String {
                        ...
                        try! realm.write {
                            ...
                            realmMessage.chatID = safeMatchID
    ...
    func sendChatNotification(_ message: String) {
        
        let myFunctions = Functions.functions()
        
        let data: [String: Any] = [
            "receiverID": matchDetails?[0].fcmToken,
            "senderName": ownMatch.name,
            "chatText": message
        ]
        
        myFunctions.httpsCallable("sendChatMessageNotification").call(data) { result, error in
